...
export function ServicesSection({ lang }: ServicesSectionProps) {
  ...
  return (
    <section id="services" className="bg-background/95 py-24 relative overflow-hidden">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">

        {/* TITLE */}
        <p className="text-sm font-medium uppercase tracking-widest text-muted-foreground/90">
        ...
        <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
          {services.map((service) => (
            <div
              key={service.title}
              className="group relative flex flex-col items-start gap-4 rounded-xl border border-border bg-background p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-[#2F6DB3]"
            >
              <div className="absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-20 bg-gradient-to-br from-[#2F6DB3]/20 via-transparent to-transparent" />

              <div className="relative flex h-10 w-10 items-center justify-center rounded-lg bg-[#2F6DB3]/10 text-[#2F6DB3] transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3">
        ...
        <div className="mt-32">
          ...
          <div className="mt-16 grid gap-10 md:grid-cols-5 relative">
            {steps.map((step, i) => (
              <div key={step.number} className="relative group">
                ...
                <div className="flex flex-col items-start">
                  <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#2F6DB3]/10 text-[#2F6DB3] font-semibold transition-all duration-300 group-hover:bg-[#2F6DB3] group-hover:text-white">
                    {step.number}
                  </div>
